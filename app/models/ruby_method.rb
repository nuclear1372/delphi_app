class RubyMethod < ActiveHash::Base
  self.data = [
    ...
    {id: 15, first_priolity: 'ハッシュ', second_priolity: '値', therd_priolity: '取得', suggestion: 'assocメソッドを提案。<br>ハッシュが引数をキーとして持つとき、見つかった要素のキーと値のペアを配列として返します。<br>キーの同一性判定には eql? メソッドではなく == メソッドを使います。<br>key が見つからなかった場合は、nil を返します。', example: 'h = {<br>"colors"  => ["red", "blue", "green"],<br>"letters" => ["a", "b", "c" ]} <br>h.assoc("letters")  #=> ["letters", ["a", "b", "c"]] <br>h.assoc("foo") #=> nil'},
    {id: 16, first_priolity: 'ハッシュ', second_priolity: '削除', therd_priolity: 'すべて', suggestion: 'clearメソッドの使用を提案。<br>ハッシュの中身を空にします。<br>空にした後のselfを返します。デフォルト値の設定はクリアされません。', example: 'h = Hash.new("default value") <br>h[:some] = "some" <br>h #=> {:some=>"some"} <br>h.clear <br>h #=> {} <br>h.default #=> "default value"'},
    {id: 17, first_priolity: 'ハッシュ', second_priolity: 'コピー', therd_priolity: '同じ', suggestion: 'dupメソッドを提案。<br>selfと同じ内容を持つ新しいハッシュを返します。<br>dup は内容だけをコピーします。', example: 'h1 = {<br>"have" => "have a",<br>"as" => "as a" } <br>h2 = h1.dup <br>h2["have"] = "has" <br>h2 #=> {"have"=>"has", <br>"as"=>"as a"} <br>h1 #=> {"have"=>"have a",<br> "as"=>"as a"}'},
    {id: 18, first_priolity: 'ハッシュ', second_priolity: '削除', therd_priolity: '指定', suggestion: 'deleteメソッドを提案。<br>key に対応する要素を取り除きます。 <br>取り除くキーを指定し,取り除かれた要素の値を返します。 <br>キーに対応する要素が存在しない時には nil を返します  ', example: 'h = {<br>:ab => "some" , <br>:cd => "all"} <br>h.delete(:ab) #=> "some" <br>h.delete(:ef) #=> nil <br> h.delete(:ef){<br>|key|"#{key} Nothing"<br>} #=> "ef Nothing" <br>h #=> {:cd=>"all"}'},
    {id: 19, first_priolity: 'HASH', second_priolity: '繰り返', therd_priolity: '取り出', suggestion: 'eachメソッドを提案。<br>ハッシュのキーと値を引数としてブロックを評価します。', example: '{:a=>1, :b=>2}.each {|a| p a} <br>#=> [:a, 1] <br>#   [:b, 2] <br>{:a=>1, :b=>2}.each {|k, v| p [k, v]} <br>#=> [:a, 1] <br>#   [:b, 2]'},
    {id: 20, first_priolity: 'ハッシュ', second_priolity: '以外', therd_priolity: '特定', suggestion: 'exceptメソッドを提案。<br>引数で指定された以外のキーとその値だけを含む Hash を返します。<br>引数に指定されていて Hash に存在しないキーは無視されます。', example: 'h = { <br>a: 100, b: 200, c: 300 } <br>h.except(:a) <br># => {:b=>200, :c=>300}'},
    {id: 21, first_priolity: 'ハッシュ', second_priolity: 'バリューのみ', therd_priolity: '取得', suggestion: 'fetch_valuesメソッドを提案。<br>引数で指定されたキーに関連づけられた値の配列を返します。<br>該当するキーが登録されていない時には、ブロックが与えられていればそのブロックを評価した値を返します。<br>ブロックが与えられていない時は KeyError が発生します。<br>self にデフォルト値が設定されていても無視されます（挙動に変化がありません）。', example: 'h = { <br>"cat" => "feline", <br>"dog" => "canine", <br>"cow" => "bovine" } <br>h.fetch_values("cow", "cat") <br># => ["bovine", "feline"] <br>h.fetch_values("cow", "bird") <br># raises KeyError'},
    {id: 22, first_priolity: 'ハッシュ', second_priolity: '検索', therd_priolity: '調べ', suggestion: 'selectメソッドを提案。<br>key, value のペアについてブロックを評価し，真となるペアだけを含むハッシュを生成して返します。', example: 'h = {<br> "a" => 100,<br> "b" => 200,<br> "c" => 300 } <br>h.select {|k,v| k > "a"}  <br>#=> {"b" => 200, "c" => 300} <br>h.select {|k,v| v < 200}  <br>#=> {"a" => 100}'},
    {id: 23, first_priolity: 'ハッシュ', second_priolity: 'キー', therd_priolity: '確か', suggestion: 'key?メソッドを提案。<br>ハッシュが引数をキーとして持つ時真を返します。', example: '{1 => "one"}.key?(1) <br># => true <br>{1 => "one"}.key?(2) <br># => false'},
    {id: 24, first_priolity: 'ハッシュ', second_priolity: 'バリュー', therd_priolity: '確か', suggestion: 'value?メソッドを提案。<br>ハッシュが引数を値として持つ時真を返します。<br>値の一致判定は == で行われます。', example: '{1 => "one"}.value?("one") <br>#=> true <br>{1 => "one"}.value?("two") <br>#=> false'},
    {id: 25, first_priolity: 'ハッシュ', second_priolity: '全てのキー', therd_priolity: '取得', suggestion: 'keysメソッドを提案。<br>全キーの配列を返します。', example: 'h1 = { <br>"a" => 100, <br>2 => ["some"], <br>:c => "c" }<br>h1.keys #=> ["a", 2, :c]'},
    {id: 26, first_priolity: 'ハッシュ', second_priolity: '数', therd_priolity: '要素', suggestion: 'lengthメソッドを提案。ハッシュの要素の数を返します。', example: 'h = { <br>"d" => 100,<br> "a" => 200,<br> "v" => 300,<br> "e" => 400 } <br>h.length #=> 4'},
    {id: 27, first_priolity: 'ハッシュ', second_priolity: '結合', therd_priolity: '別の', suggestion: 'mergeメソッドを提案。<br>ハッシュの内容を順番にマージ(統合)した結果を返します。', example: 'h1 = { "a" => 100,"b" => 200 } <br>h2 = { "b" => 246, "c" => 300 } <br>h3 = { "b" => 357,"d" => 400 } <br>h1.merge #=> {"a"=>100, "b"=>200} <br>h1.merge(h2) <br>#=> {"a"=>100, "b"=>246, "c"=>300} <br>h1.merge(h2, h3)  <br>#=> {"a"=>100, "b"=>357, "c"=>300, "d"=>400}'},
    {id: 28, first_priolity: 'ハッシュ', second_priolity: '配列', therd_priolity: '変', suggestion: 'to_aメソッドを提案。<br>キーと値からなる 2 要素の配列を並べた配列を生成して返します。', example: 'h1 = { <br>"a" => 100,<br> 2 => ["some"],<br> :c => "c" } <br>h1.to_a <br>#=> [["a", 100],<br> [2, ["some"]],<br> [:c, "c"]]'},
    {id: 29, first_priolity: 'ハッシュ', second_priolity: '全てのバリュー', therd_priolity: '取得', suggestion: 'valuesメソッドを提案。<br>ハッシュの全値の配列を返します。', example: 'h1 = { <br>"a" => 100,<br> 2 => ["some"],<br> :c => "c" } <br>h1.values <br>#=> [100, ["some"], "c"]'}
  ]
